...
for filename in files:
    if (filename[-5:] == ".json" and filename.find("R10")!=-1 and filename.find("R100")==-1) :
...
for filename in listOfFiles:
    ...
    averageData[granularity_log2].append(tuple((interval,averageBandwidth)))      # locate in dictionary the granularity bin
    aggregateData[granularity_log2].append(tuple((interval,aggregateBandwidth)))
...
for key, value in aggregateData.items():
    ...
    for tup in sorted(value):
        x_interval.append(tup[0])
        y_bandwidth.append(tup[1])
    
    ax_aggregate.scatter(x_interval, y_bandwidth, s=4, label=granularity)
    ax_aggregate.plot(x_interval, y_bandwidth)
...
for key, value in averageData.items():
    ...
    ax_average.scatter(x_interval, y_bandwidth, s=3, label=granularity)
    ax_average.plot(x_interval, y_bandwidth)


ax_aggregate.set_xlabel("Txn Arriving Interval (ns)")
ax_aggregate.set_ylabel("Bandwidth (GB/s)")
ax_aggregate.legend(loc='upper right')

ax_average.set_xlabel("Txn Arriving Interval (ns)")
